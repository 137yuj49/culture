...
//====다 체크하고 submit
	function check(){
		var sabun = iform.member_id.value;
		var name = iform.member_name.value;
		var pwd = iform.member_pwd.value;
		var pwd2 = iform.confirmPassword.value;
...
//====사번 체크
	function checkSb(){//사번 입력체크하고 중복확인 팝업창 열어주기
		...
		label.innerHTML = "";
		open('openSb.jsp','oepnSb','width=450, height=180, left=100, top=200');
	}//checkSb end
	
	function flagS_t(){//사번 플래그를 true로 바꿔주는 메소드(팝업창에서 작동)
		flagS=true;
		subform.sabun.value = opener.iform.sabun.value;
	}
...
//====우편번호
	function DaumPostcode() {
      ...
      oncomplete: function(data) {
        ...
        document.getElementById('b_juso1').value = fullAddr;

        // 커서를 상세주소 필드로 이동한다.
        document.getElementById('b_juso2').focus();
      }
		}).open();
	}//end
